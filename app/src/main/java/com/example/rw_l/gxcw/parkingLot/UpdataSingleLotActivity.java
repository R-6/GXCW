...
public class UpdataSingleLotActivity extends AppCompatActivity {

    @BindView(R.id.toolbar_updata_single_lot)
    Toolbar toolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_updata_single_lot);
        ...
        QMUIGroupListView groupListView = findViewById(R.id.groupListView_updata_single_lot);

        itemView1 = groupListView.createItemView(null, "车位名称", name, QMUICommonListItemView.HORIZONTAL, 1);
        if (itemView1.getDetailText().length()<=0) itemView1.showRedDot(true);
        itemView2 = groupListView.createItemView(null, "车位地址", location, QMUICommonListItemView.VERTICAL, 1);
        if (itemView2.getDetailText().length()<=0) itemView2.showRedDot(true);
        itemView3 = groupListView.createItemView(null, "可用开始时间", start, QMUICommonListItemView.HORIZONTAL, 1);
        if (itemView3.getDetailText().length()<=0) itemView3.showRedDot(true);
        itemView4 = groupListView.createItemView(null, "可用结束时间", end, QMUICommonListItemView.HORIZONTAL, 1);
        ...
        QMUIGroupListView.newSection(this)
                .addItemView(groupListView.createItemView(null,"保存",null,1,0), new View.OnClickListener() {
                    public void onClick(View v) {
                        ...
                        returnBundle.putString("start",itemView3.getDetailText().toString()+"");
                        returnBundle.putString("end",itemView4.getDetailText().toString()+"");
    ...
    private void changeData(QMUICommonListItemView itemView) {
        ...
        dialogChange.addAction("确定", new QMUIDialogAction.ActionListener() {
            ...
            public void onClick(QMUIDialog dialog, int index) {
                ...
                if (str.length() <= 0)
                    Toast.makeText(UpdataSingleLotActivity.this, "请输入"+myitemView.getText().toString(), Toast.LENGTH_SHORT).show();
                else {
        ...
        dialogBack.addAction("退出", new QMUIDialogAction.ActionListener() {
            ...
            public void onClick(QMUIDialog dialog, int index) {
                if(dialogBack.isChecked()){
                    ...
                    returnBundle.putString("start",itemView3.getDetailText()+"");
                    returnBundle.putString("end",itemView4.getDetailText()+"");

                    intent.putExtras(returnBundle);
                    setResult(1, intent);
                    UpdataSingleLotActivity.this.finish();
                }else UpdataSingleLotActivity.this.finish();
            }
        });
        dialogBack.show();
    }
